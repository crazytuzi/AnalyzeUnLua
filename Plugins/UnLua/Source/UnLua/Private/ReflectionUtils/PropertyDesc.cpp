...
public:
    ...
    {
        // 根据UProperty的ArrayDim分成两种情况，像int a;这样定义的属性，ArrayDim为1，而像int a[20]这样定义的属性，其实是个数组，ArrayDim为20
        ...
        {
            // 使用lua_pushinteger接口把数值push到lua栈中即可，这样就会创建一个lua的number变量了
 ...
 * FObjectPropertyDesc不仅处理UObject指针，还处理WeakObjectProperty和LazyObjectProperty
 */
...
public:
    ...
    virtual void GetValueInternal(lua_State *L, const void *ValuePtr, bool bCreateCopy) const override
    {
        ...
        else
        {
            ...
            {
                // 一个UObject如果与lua进行了绑定，那么lua中会有一张对应的table，该UObject指针在lua中对应的数据就是这个table
                // 绑定及table创建可见NewLuaObject函数，table被创建后，会在lua中被保存到"ObjectMap"表中进行记录，键为该UObject的地址，值为table
                // 而如果UObject没有实现UnLuaInterface或没有被动态绑定，那Object本身与lua没有关系，在lua中不会创建table
                // 这个UObject被传递到lua中时，会创建一个UserData，UserData值就是UObject的地址，并且设置metatable为对应Class的ClassMetatable
...
public:
    ...
    virtual void GetValueInternal(lua_State *L, const void *ValuePtr, bool bCreateCopy) const override
    {
        // TArray传到Lua时有两种创建FLuaArray方式，一种会把TArray复制一份，Lua中对FLuaArray操作不会影响C++TArray
        // 另一种是把TArray引用传到Lua，Lua对FLuaArray的添加和删除会影响C++的TArray
        if (bCreateCopy)
        {
            // 需要再申请一块FScriptArray内存，并且在UserData被GC时释放它
        ...
        else
        {
            ...
            // 去全局表"ScriptContainerMap"中查找该ScriptArray对应的UserData，没有就创建一个
            // 创建后会把Metatable设置为FScriptContainerDesc::Array，即"TArray"，并且注册到"ScriptContainerMap"中
            void *Userdata = CacheScriptContainer(L, ScriptArray, FScriptContainerDesc::Array);
            // 在创建好UserData后，就会在这个UserData的内存上调用FLuaArray的构造函数，进行初始化
            if (Userdata)
            {
 ...
 * Struct也支持反射，因此UStruct会在lua中进行注册，创建ClassMetatable，在lua中也能访问FStruct的属性
 * 同样的，FStruct本身就需要能在C++和lua间传递。FStruct的处理会简单些，因为它不会与lua table绑定，lua中与其对应的数据类型只会是UserData
 */
...
public:
    ...
    virtual void GetValueInternal(lua_State *L, const void *ValuePtr, bool bCreateCopy) const override
    {
        ...
        else
        {
            ...
            {
                // FStruct并不是指针，因此可以选择把FStruct整个复制到lua中，作为一个UserData，并设置ClassMetatable，这样lua的修改就不会影响原属性
                // 平常访问属性的操作，都会使用原始地址，并不会复制一份
...
public:
    ...
    virtual bool SetValueInternal(lua_State *L, void *ValuePtr, int32 IndexInStack, bool bCopyValue) const override
    {
        ...
        if (FuncIdxInTable != INDEX_NONE)
        {
            ...
            {
                // 如果此Callback在Callbacks容器中有对应的UFunction，则说该Class之前有其他Delegate绑定过这个lua方法，可以不用再创建新的UFunction，但是需要更新Signature里的计数器
