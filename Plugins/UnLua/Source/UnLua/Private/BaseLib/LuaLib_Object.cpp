...
/**
 * GC function
 * Object通过属性访问传到lua和绑定table都会对UObject添加引用，那么table和userdata被lua gc时自然要去掉引用
 * UnLua通过元表设置，让这些table和userdata被gc时可以调用到UObject_Delete方法，然后在其中就会去掉ReferencedObjects数组中的引用
 */
...
/**
 * Export UObject
 * BEGIN_EXPORT_REFLECTED_CLASS(UObject)，定义会转到BEGIN_EXPORT_CLASS_EX宏，这是一个通用的Class导出实现
 * 定义了一个struct，宏展开后名称为FExportedUObjectHelper，内部包含静态类型StaticInstance和TExportedClass类型属性ExportedClass，
 * 然后是析构函数与一半构造函数。构造函数中会创建TExportedClass类型变量ExportedClass，并执行ExportClas函数进行导出
 * ADD_LIB(UObjectLib)，往ExportedClass里注册函数
 * END_EXPORTED_CLASS()，结束构造函数和类定义
 * IMPLEMENT_EXPORTED_CLASS 创建静态变量StaticInstance
 */
