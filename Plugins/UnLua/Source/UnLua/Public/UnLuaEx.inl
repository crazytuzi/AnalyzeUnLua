...
namespace UnLua
{
    ...
    struct TInvokingHelper
    {
        ...
        {
            // 调用导出函数
            // 调用TExportedMemberFunction的Func成员变量，接着通过lambda调用到我们的导出函数，并得到返回值
            RetType RetVal = UnLua::Invoke(Func, Args, typename TZeroBasedIndices<sizeof...(ArgType)>::Type());
            // 引用传递参数压栈
            // C++函数只有一个返回值，lua支持多返回值，而静态导出函数会把非const引用传递参数作为函数额外的返回值压入luastack
            // 比如int &a这种参数，这样就实现了多返回值效果
            // 压栈依然采用静态导出常用的UnLua::Push实现。
            int32 Num = PushNonConstRefParam<ArgType...>(L, Args, ParamIndices);
            // 函数返回值压栈
            // 在处理完引用传递参数，最后会把函数返回值压栈，注意返回值是后压栈的，lua代码接收结果时要把C++函数返回值放最后
    ...
    {
        // InvokeFunction作为中转函数，会先取出upvalue，然后调用TExportedMemberFunction::Invoke方法
    ...
    */
    ...
    {
        // 把InvokeFunction设置成全局变量，这样lua中就能直接调用此方法
        lua_pushlightuserdata(L, this);
        lua_pushcclosure(L, InvokeFunction, 1);
    ...
    {
        // push到lua的函数为InvokeFunction和一个upvalue TExportedMemberFunction构成的closure
        // make sure the meta table is on the top of the stack
    ...
    /**
     * 因为AddProperty时我们已经知道了属性在class中偏移，因此可以把class实例地址加上偏移得到属性地址
     * 另外Read是模板函数，属性类型T作为模板参数，在AddProperty时指定
     * 因此对于不同的属性类型，会生成各自的Read函数实例，即属性类型是确定的
     * 有了类型和内存偏移信息，就能获取该属性了，由函数UnLua::Push实现
     */
    template <typename T>
    ...
    // Write是模板函数，T为属性类型，等号左边获取了属性在对象中的类型偏移，右边为与UnLua::Read类似的UnLua::Get模板函数
    template <typename T>
    ...
    void TExportedClassBase<bIsReflected>::AddLib(const luaL_Reg *InLib)
    {
        if (InLib)
        {
            ...
            {
                // 根据函数名称和地址创建GlueFunction，然后把GlueFunction加到GlueFunctions数组，原生C模块添加的函数都称为“GlueFunction”
