...
/**
 * Register a class 
 * 通过对象名，用UE4的反射获取这个对象类型，然后执行注册，即记录反射信息到UnLua的反射库中
 *
...
bool FReflectionRegistry::AddOverriddenFunction(UFunction *NewFunc, UFunction *OverriddenFunc)
{
    ...
    {
        // 存储了原UFunction和CopyUFunction的键值对，之后有需要可以在里面查找并调用原UFunction
...
/**
 *
 * 创建一个UnLua反射库的FClassDesc，用于记录UE4 UStruct类型信息，用作后续反射，
 * 然后记录类型名字、Struct、FClassDesc之间的映射关系，方便后续取用，同时递归它的父类，做相同的注册。
 * 这样做的原因，就是记录传入类型名对应的UE4反射信息到UnLua自己的反射库中，目的是为了后续Lua、C++交互调用的方便。
 */
